...
nhis_enc$inv_wt <- (1 / nhis$svywt)^2
```
...
init_net = function(data) {
  predict(net, data, type = "raw")[, 1]
}
```
...
```{r}
ridge1 = LearnerResidualFitter$new(lrn("regr.glmnet", alpha = 0, lambda = 2 / nrow(post)))

ridge2 = LearnerResidualFitter$new(lrn("regr.glmnet", alpha = 0, lambda = 2 / nrow(post)))
...
mc1 = MCBoost$new(init_predictor = init_rf, 
                 subpop_fitter = ridge1,
                 multiplicative = TRUE,
                 partition = TRUE,
                 max_iter = 15)
mc1$multicalibrate(d, l)

mc2 = MCBoost$new(init_predictor = init_rf, 
                 subpop_fitter = pops,
                 partition = TRUE,
                 max_iter = 15)
mc2$multicalibrate(d, l)

mc3 = MCBoost$new(init_predictor = init_net, 
                 subpop_fitter = ridge2,
                 multiplicative = TRUE,
                 partition = TRUE,
                 max_iter = 5)
mc3$multicalibrate(d, l)

mc4 = MCBoost$new(init_predictor = init_net, 
                 subpop_fitter = pops,
                 partition = TRUE,
                 max_iter = 5)
mc4$multicalibrate(d, l)
```
...
test$net <- predict(net, newdata = test, type = "raw")[, 1]
...
eval <- map(grouping_vars, group_by_at, .tbl = test) %>%
  map(summarise,
      ...
      'bias_net' = abs(mean(net) - mean(label))*100,
