...
- A training set `train` for training the initial prediction models (75 \% of data)
- An auditing set `post` for post-processing the initial models with MCBoost
- A test set `test`for model evaluation
...
We initialize two custom auditors for MCBoost: Ridge regression with a small penalty on model complexity, and a `SubpopFitter` with a fixed set of subpopulations.
...
nnet_mc1 = MCBoost$new(init_predictor = init_nnet,
                       ...
                       partition = TRUE,
                       max_iter = 15)
nnet_mc1$multicalibrate(d1, l)

nnet_mc2 = MCBoost$new(init_predictor = init_nnet,
                       subpop_fitter = pops,
                       partition = TRUE,
                       max_iter = 15)
nnet_mc2$multicalibrate(d2, l)
```
...
lasso = LearnerResidualFitter$new(lrn("regr.glmnet", alpha = 1, lambda = 40 / nrow(post)))
```
