...
We only use the first 200 samples of the *sonar* data set to train our multi-calibrated
model.
...
## Example 5: Multi-Callibration Regression

We abuse the `Adult` dataset in order to showcase how `mcboost`
...
adult_train$Target = ifelse(adult_train$Target == ">50K", runif(1, 50, 100), runif(1, 0, 50))
# In order to work with mcboost, we have to scale our target
# to be between 0 and 1.
adult_train$Target  = adult_train$Target  / 100

train_tsk = TaskRegr$new("adult_train", adult_train, target = "Target")
```
...
adult_test$Target = ifelse(adult_test$Target == ">50K", runif(1, 50, 100), runif(1, 0, 50))
# In order to work with mcboost, we have to scale our target
# to be between 0 and 1.
adult_test$Target  = adult_test$Target  / 100
...
test_tsk = TaskRegr$new("adult_test", adult_test, target = "Target")
prep_test = pipe$predict(test_tsk)[[1]]
```
...
```{r}
mean((round(prs) - one_hot(test_labels)[, 2])^2)
```
