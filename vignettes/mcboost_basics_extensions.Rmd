...
### 2.3 Multiaccuracy Boosting

Multi-accuracy boosting as defined in Kim et al., 2020 involes using three probability sub-populations:
The whole population X, the positively labeled observations X_1 and the negatively labeld population X_0.

This can be easily achieved by setting `num_buckets = 2` (the default) and enabling `partition=True`


```{r}
rf = LearnerResidualFitter$new(lrn("regr.rpart", minsplit = 5L))
mc = MCBoost$new(subpop_fitter = rf, partition=TRUE, num_buckets=2L)
mc$multicalibrate(data, labels)
```

### 2.4 MCBoost with non-mlr3 models: GLM
...
### 2.5 Avoiding Overfitting in MCBoost - CVLearner

Very often `MCBoost`'s calibration is very agressive and tends to overfit.
This section tries to introduce a method to regularize against this overfitting, namely using a
`Cross-Validated` learner that predicts on held-out data during the training phase.
Other, simpler methods include choosing a smaller `eta` or reducing the number of `iters`.
...
As an `init_predictor` we again use the `glm` model from above.
...
Note, that this can sometimes take a little longer since each learner is cross-validated using `3` folds.

```{r}
d = data[, -1]
l = data$Class
mc = MCBoost$new(init_predictor = init_predictor, subpop_fitter=CVTreeResidualFitter$new())
mc$multicalibrate(d[1:200,], l[1:200])
mc$predict_probs(d[201:208,])
```