...
prep_task$set_col_roles(c("Race.Amer.Indian.Eskimo", "Race.Asian.Pac.Islander", "Race.Black", "Race.Other", "Race.White", "fnlwgt"), remove_from = "feature")
```
...
adult_test$accmc = (round(prs) == test_labels)
adult_test[, .(mean(accmc), .N), by = .(Race)]
# Get accuracy per subgroup for initial predictor
adult_test$accinit = (round(init_predictor(test_data)) == test_labels)
adult_test[, .(mean(accinit), .N), by = .(Race)]

# mask = as.logical(test_data$Race.Black)
# mean(round(prs[mask]) == test_labels[mask])
# mean(round(init_predictor(test_data)[mask]) == test_labels[mask])
```
...
test_data[, .(mcboost = mean(se_mcboost), initial = mean(se_init), .N), by = .(HispPerCap > 0.5)]
```

So we also improved across the subgroups.
