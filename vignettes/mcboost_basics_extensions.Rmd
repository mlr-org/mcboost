...
Internally mcboost runs the following procedure `max_iters` times:

1. Predict on X using the model from the previous iteration, `init_predictor` in the first iteration.
1. Compute the residuals `r = y - y_hat`
1. Split predictions into `num_buckets` acording to `y_hat`.
1. Fit the auditor (`subpop_fitter`) (here called`c(x)`) on the data in each bucket with target variable `r`.
1. Compute `misscal = mean(c(x) * r(x))`
1. if `misscal > alpha`:
...
Multi-calibration is an iterative prodedure.
The `t` parameter can be used to predict using only the first `t` iterations.
This then predicts using only the first `t` iterations of the multi-calibration proceedure.
...
### Multi-Calibrating data with missing values using a pipeline
...
mc = MCBoost$new(subpop_fitter = auditor, init_predictor = init_model)
