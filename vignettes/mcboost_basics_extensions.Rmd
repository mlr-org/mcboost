...
```{r}
mc = MCBoost$new(subpop_fitter = "RidgeAuditorFitter", init_predictor = init_predictor)
...
d = data[, -1]
l = data$Class
mc = MCBoost$new(init_predictor = init_predictor, subpop_fitter=CVTreeAuditorFitter$new(), max_iter = 2L)
mc$multicalibrate(d[1:200,], l[1:200])
mc$predict_probs(d[201:208,])
```
...
mc = MCBoost$new(
  ...
  subpop_fitter= TreeAuditorFitter$new(),
...
```{r}
mc = MCBoost$new(subpop_fitter = "RidgeAuditorFitter", init_predictor = init_predictor, eta = 0.1)
mc$multicalibrate(data, labels)
```
